...
class Mage_Catalog_Block_Product_List_Toolbar extends Mage_Core_Block_Template
{
    ...
    public function setCollection($collection)
    {
        ...
        if ($this->getCurrentOrder()) {
            if($this->getCurrentOrder() == "budge") {
                /*
                $this->_collection->addAttributeToSelect(
                    "SELECT sum(value) as budgesum FROM `budge_user` as A join `budge` as B on A.budge_id = B.budge_id where A.`customer_id` = `creator_id`"
                )
                ->setOrder("budgesum", $this->getCurrentDirection());
                */
            }
            else {
