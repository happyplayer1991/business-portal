...
class Cminds_Supplierfrontendproductuploader_Block_Product_Create_Categories extends Cminds_Supplierfrontendproductuploader_Block_Product
{
    ...
    public function listCategory($categories) {
        $string = '';
        $customer_categories_str = Mage::getSingleton('customer/session')->getCustomer()->getCategories();
        $customer_categories = array_map('intval', explode(',', $customer_categories_str));
        ...
        foreach($categories AS $category) {
            ...
            if($cat->getId() && $cat->getName() != '' && $cat->getId() != 1) {
                if((in_array($cat->getId(), $customer_categories) && ($cat->getLevel() == 3)) || ($cat->getLevel() != 3))
