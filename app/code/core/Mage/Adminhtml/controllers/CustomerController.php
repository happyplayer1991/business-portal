...
class Mage_Adminhtml_CustomerController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function getSuppliersAction()
    {
        ...
        $suppliers = Mage::getModel('customer/customer')->getCollection()
        ...
        foreach($suppliers as $supplier) :
            ...
            array_push($suppliers_data, $tree_data);
        endforeach;
    ...
    public function SetAction()
    {
        ...
        $postdata = $this->getRequest()->getPost();
        $datas = $postdata['budges'];
        foreach($datas as $data):
            // get customer by id from post data
            $customer = Mage::getModel('customer/customer')->load($data['id']);
            // get customer budges from post data
            $budge_ids  = explode(",", $data['budge']);
            
            //remove old budge data
            $connection->delete("budge_user","customer_id = ".$data['id']);
            if(!empty($budge_ids)) {
                $array = array();
                // insert budge of supplier
                foreach($budge_ids as $budge_id):
                    // $query = "INSERT INTO budge_user (customer_id, budge_id) VALUES (:customer_id, :budge_id)";
                    if((intval($budge_id) != -1) && (intval($budge_id) != 0) ) { 
                        $array[] = [
                            'customer_id' => $data['id'],
                            'budge_id' => $budge_id
                        ];
                ...
                endforeach;
                $connection->insertMultiple('budge_user', $array); 
            }


            //$customer->setBudges($budge['budge']);
            //$customer->save();
        endforeach;

        //$connection->delete("budge_user","budge_id = -1");
        $connection->delete("budge_user","budge_id = 0");
        //echo 'success';
