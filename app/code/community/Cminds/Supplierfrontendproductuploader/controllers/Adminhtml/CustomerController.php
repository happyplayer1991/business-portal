...
class Cminds_Supplierfrontendproductuploader_Adminhtml_CustomerController extends Mage_Adminhtml_CustomerController {
    ...
    public function saveAction()
    {
        ...
        if ($data) {
            ...
            $errors = null;
            if (method_exists(Mage::helper('customer'), 'getIsRequireAdminUserToChangeUserPassword')) {
            ...
            try {
                 ...
                 */
                if (Mage::helper('supplierfrontendproductuploader')->isSupplierNeedsToBeApproved() && $isSupplier) {
                    $customerOrig = $customer->getOrigData();
                    $customerCurr = $customer->getData();

                    if (isset($customerOrig['supplier_approve'])
                        && $customerOrig['supplier_approve'] == 0
                        && $customerCurr['supplier_approve'] == 1
                    ) {
                        Mage::helper('supplierfrontendproductuploader/email')
                            ->notifySupplierWhenApproved($customer->getId());
                    } elseif (!isset($customerOrig['supplier_approve'])
                        && $customerCurr['supplier_approve'] == 1
                    ) {
                        Mage::helper('supplierfrontendproductuploader/email')
                            ->notifySupplierWhenApproved($customer->getId());
        ...
        if ($customer->getId()) {
            try {
                ...
                if ($this->getRequest()->getParam('supplier', false)) {
                    Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__('The supplier has been deleted.')
                    );
                } else {
                    Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__('The customer has been deleted.')
                    );
                }
            } catch (Exception $e) {
