...
(function($) {
	$.fn.PavMegamenuEditor = function(opts) {
		...
		function listenEvents( $megamenu ){
			 ...
			 */
			 ...
			 $("#menu-form .add-col").click( function(){
		 		if ( activeMenu ){ 
		 			var num = 6; 
		 			var col = $( '<div class="col-sm-'+num+' mega-col active"><div class="inner"></div></div>'  );
 			 ...
 			 $("#btn-inject-widget").click( function(){
 				...
 				if( wid > 0 ){
		 			...
		 			$(col).children('div').html('<div class="loading">Loading....</div>');
 				 	$.ajax({
						url: config.action_widget+"widgets/"+$(col).data( 'widgets'),
						data:'',
						type:'GET',
						}).done(function( data ) {
 			 ...
 			 * inject widgets
 			 */
 			 $("#btn-inject-submenu").click( function(){
 			 	var col = $( ".mega-col.active", activeMenu );
 			 	if(col.length > 0 && col.data("type") != "menu"){
 			 		col.data("type", "menu");
 			 		var _s = $('<div class="ves-submenu">Sub Menu Items</div>');
	    	 	 	$(col).find(".inner").append(_s);
 			 	} else {
 			 		alert( 'You can not inject more sub menu items.' );
 			 	}
 			  ...
 			  */
 			  ...
 			  $($megamenu).delegate( '.ves-submenu', 'hover', function(){ 
		 		//$(".row",$megamenu).removeClass('active');
		 		// $(this).addClass('active'); 
	    	 	 var sm = $(this); 
 			...
 			$($megamenu).delegate( '.level1 > .dropdown-menu-inner', 'hover', function(){
		 		//$(".row",$megamenu).removeClass('active');
		 		// $(this).addClass('active');
		 		$( this ).find(".row").sortable({handle: $(this).find('.w-move'),
		    	 	 								connectWith:".level1 > .dropdown-menu-inner .row",
		    	 	 								placeholder: "portlet-placeholder ui-corner-all pull-left",
		    	 	 								scroll: false,
		    	 	 								appendTo: 'body',
			    ...
			    $( this ).find(".row").disableSelection();
	    	 });
 			  $($megamenu).delegate( '.mega-col', 'hover', function(){
		 		//$(".row",$megamenu).removeClass('active');
		 		// $(this).addClass('active');
		 		if( $(this).find('.w-move').length<= 0 ){
		 			if($( this ).parent().parent().parent().hasClass("level1")) {
		 				var _m = $('<span class="w-move" title="Drag n Drop the Column to change position."></span>');
		    	 	 	$(this).append(_m);

		 			}
	    	 	 	
	    	 	 }
		 		if(typeof ($(this).data("type")) == "undefined" || $(this).data("type") != "menu") {
		    	 	$( this ).find(".inner").sortable({handle: $(this).find('.ves-widget'), 
		    	 							connectWith:".mega-col .inner", 
		    	 							...
		    	 							update: function( event, ui ) {
									    	 	 	var sort_items = [];
										 		 	$(".ves-widget", $(ui.item).parent()).each(function(){
										 		 		var dws = $(this).data("id");
										 		 		sort_items.push(dws);
										 		 	})
										 		 	if(sort_items.length > 0) {
										 		 		$(ui.item).parent().parent().data('widgets',sort_items.join("|"));
										 		 	}

				 							}});
		    		$( this ).find(".inner").disableSelection();
	    	 	}
	    	 });
 			  $($megamenu).delegate( '.ves-widget', 'hover', function(){
		 		//$(".row",$megamenu).removeClass('active');
		 		// $(this).addClass('active');
	    	 	 ...
	    	 	 if( $(this).find('.w-setting').length<= 0 ){
	    	 	 	...
	    	 	 	_s.click( function(){
	    	 	 		...
	    	 	 		var dws = col.data('widgets')+"|";
	    	 	 	 	var dws = dws.replace( $(w).data('id')+"|",'' ).replace(/\|$/,'');
	    	 	...
	    	 	if(typeof($(this).attr("title")) == "undefined" || $(this).attr("title") == "") {
		 			$(this).attr("title", "Drag n Drop the Widget to change position.");
		 		}
	    	 	
	    	 	/*
	    	 	$( this ).parent().sortable({handle: $(this).find('.w-move'), connectWith:".mega-col .inner", placeholder: "portlet-placeholder ui-corner-all", update: function( event, ui ) {
		    	 	 	var sort_items = [];
			 		 	$(".ves-widget", $(ui.item).parent()).each(function(){
			 		 		var dws = $(this).data("id");
			 		 		sort_items.push(dws);
		...
		function recalculateColsWidth(){
	 	...
	 	function checkWidgetExists(array_widgets, current_id) {
	 		var checked = false;
	 		if(array_widgets.length > 0) {
				for(i=0;i< array_widgets.length;i++) {
					if(array_widgets[i] == current_id) {
						checked = true;
						break;
					}
				}
	 	 	 ...
	 	 	 $("#megamenu-content #mainmenutop li.parent").each( function() {
				 	...
				 	$(this).children('.dropdown-menu').children('div').children('.row').each( function(){
			 			...
			 			$(this).children(".mega-col" ).each( function(){
			 				$( this ).children('.inner').sortable( "destroy" );
			 				
			 				var col_widgets = $(this).data("widgets");
			 				if(col_widgets) {
			 					if($(this).find(".ves-widget").length > 0){
			 						var tmp_array_widgets = col_widgets.split("|");
			 						var tmp_widgets = [];

			 						$(this).find(".ves-widget").each(function(){
			 							

			 							if(checkWidgetExists(tmp_array_widgets, $(this).data("id"))) {
			 								tmp_widgets.push($(this).data("id"));
			 							}
			 						});
			 						
			 						$(this).data("widgets", tmp_widgets.join("|"));
			 					} else {
			 						$(this).data("widgets", null);
			 						$(this).removeData("widgets");
			 					}
	 	...
	 	function loadWidgets(){
	 		...
	 		$.each( ajaxCols, function (i, col) {
	 			...
	 			$.ajax({
					...
					data:'widgets='+$(col).data( 'widgets')+'&form_key='+ window.FORM_KEY,
					type:'POST',
					}).done(function( data ) {
			 		col.children('div').html( data );
			 		cnt++;
